...
object Telephone extends ScalesApp(500, 600) {
  val buttons = ((button("1", Colors.Orange) -|| button("2", Colors.Green)) -^
                 (button("3", Colors.Yellow) -|| button("4", Colors.Purple)) -^
                 button("Flash", Colors.Red, 240)
                ) -||
                (button("Police", Colors.Red) -^
                    button("Taxi", Colors.Blue) -^
                    button("Doctor", Colors.Yellow))
  
  def button(name: String, color: Color, width: Int = 100, height: Int = 100): Graphic =
    (Text(name, FontSize(28)).center -& Name(name, Rectangle(width, height).pad(20).center -* color)).topLeft
  ...
  def render: State => Graphic = {
    case state => buttons -^ Text(state, FontSize(48)).pad(40)
  }
